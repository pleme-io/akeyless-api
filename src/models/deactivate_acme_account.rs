/*
 * Akeyless API
 *
 * The purpose of this application is to provide access to Akeyless API.
 *
 * The version of the OpenAPI document: 3.0
 * Contact: support@akeyless.io
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// DeactivateAcmeAccount : deactivateAcmeAccount is a command that Deactivates \\ Deletes an acme external account
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DeactivateAcmeAccount {
    /// The acme account id to deactivate
    #[serde(rename = "acme-account-id")]
    pub acme_account_id: String,
    /// The name of the PKI certificate issuer
    #[serde(rename = "cert-issuer-name")]
    pub cert_issuer_name: String,
    /// Delete the account
    #[serde(rename = "delete-account", skip_serializing_if = "Option::is_none")]
    pub delete_account: Option<bool>,
    /// Set output format to JSON
    #[serde(rename = "json", skip_serializing_if = "Option::is_none")]
    pub json: Option<bool>,
    /// Authentication token (see `/auth` and `/configure`)
    #[serde(rename = "token", skip_serializing_if = "Option::is_none")]
    pub token: Option<String>,
    /// The universal identity token, Required only for universal_identity authentication
    #[serde(rename = "uid-token", skip_serializing_if = "Option::is_none")]
    pub uid_token: Option<String>,
}

impl DeactivateAcmeAccount {
    /// deactivateAcmeAccount is a command that Deactivates \\ Deletes an acme external account
    pub fn new(acme_account_id: String, cert_issuer_name: String) -> DeactivateAcmeAccount {
        DeactivateAcmeAccount {
            acme_account_id,
            cert_issuer_name,
            delete_account: None,
            json: None,
            token: None,
            uid_token: None,
        }
    }
}

