...
**audience** | Option<**String**> | Token audience for Splunk token creation (relevant only for rotator-type=token) | [optional]
...
**expiration_date** | Option<**String**> | Token expiration date in YYYY-MM-DD format (relevant only for rotator-type=token when manual rotation is selected). Time will be set to 00:00 UTC. | [optional]
**hec_token** | Option<**String**> | For rotator-type=hec-token, optionally set/replace the stored HEC token value. | [optional]
...
**splunk_token** | Option<**String**> | For rotator-type=token, optionally set/replace the stored Splunk authentication token value. | [optional]
**token** | Option<**String**> | Authentication token (see `/auth` and `/configure`) | [optional]
**token_owner** | Option<**String**> | For rotator-type=token, specify the Splunk username that the new token should be issued for. (Splunk 10 requires name=<token-owner> when creating auth tokens.) | [optional]
